...
export default function Stock() {
  ...
  const [modal, setModal]         = useState(null)   // 'nuevo' | 'editar' | 'excel'
  ...
  const importarExcel = async (e) => {
    ...
    try {
      ...
      mostrarToast(e.response?.data?.detail || 'Error al importar', 'error')
    }
    e.target.value = ''
  }
  ...
  return (
    <div className="flex flex-col h-full p-4 gap-4">
      ...
      <div className="bg-slate-800/50 border border-slate-700/40 rounded-xl p-3 text-xs text-slate-500">
        <span className="text-slate-400 font-medium">Formato Excel:</span>{' '}
        Columnas: <span className="text-slate-300 font-mono">codigo_barra | nombre | precio_costo | precio_venta | stock | stock_minimo | categoria</span>
      </div>
      ...
      <div className="flex-1 overflow-auto">
        {loading ? (
          <div className="text-center text-slate-500 py-16 text-sm animate-pulse-soft">Cargando productos...</div>
        ) : (
              ...
              {productos_filtrados.map(p => (
                <tr key={p.id} className={`hover:bg-slate-800/50 transition-colors ${p.stock_bajo ? 'bg-amber-950/20' : ''}`}>
                  ...
                  <td className="py-2.5 px-2 text-right">
                    <span className="text-green-400 font-medium">+{p.ganancia_pct}%</span>
                  </td>
                  <td className="py-2.5 px-2 text-right">
                    <span className={`font-semibold font-mono ${p.stock_bajo ? 'text-amber-400' : 'text-white'}`}>
                      {p.stock}
                    </span>
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 w-full max-w-md animate-fade-in">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                ...
                <input className="w-full" value={form.codigo_barra} onChange={e => setForm({...form, codigo_barra: e.target.value})} />
              </div>
              <div className="col-span-2">
